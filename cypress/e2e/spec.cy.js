  ...
  // Login
    ...
    cy.visit('https://10.10.15.50:8143/#p=config_networking_networks').then(() => {
      cy.get('.fa.fa-plus').click().then(() => {
        ...
        // Name Field
        cy.get('input[name="display_name"]', { timeout: 50000 }).type('Cypress-test-VN-auto-2')
  ...
  it('Create VxLAN VN with new LR (without Route Target)', () => {
    ...
    cy.visit('https://10.10.15.50:8143/#p=config_networking_networks').then(() => {
      cy.get('.fa.fa-plus').click().then(() => {
        ...
        // Name Field
        cy.get('input[name="display_name"]', { timeout: 50000 }).type('Cypress-test-VN-auto')
  ...
  it('Create VxLAN VN with existing LR', () => {
    ...
    cy.visit('https://10.10.15.50:8143/#p=config_networking_networks').then(() => {
      cy.get('.fa.fa-plus').click().then(() => {
        ...
        cy.contains('.select2-result-label', 'Cypress-test-VN-auto-backend-lr (VNI: 7000)').click();
    ...
    // Check for Ports creation
    cy.visit('https://10.10.15.50:8143/#p=config_net_ports').then(() => {
      cy.get('.slick-cell.l4.r4').should('contain', 'Cypress-test-VN-append')
    })

    // Check for Floating IP Pools creation
    cy.visit('https://10.10.15.50:8143/#p=config_networking_fippool').then(() => {
      cy.get('.slick-cell.l5.r5').should('contain', 'Cypress-test-VN-append')
    })

    // Check for Logical Router appending
    cy.visit('https://10.10.15.50:8143/#p=config_net_routers').then(() => {
      cy.get('.slick-cell.l2.r2').should('contain', 'Cypress-test-VN-auto-backend-lr').parent('.slick-row').find('.slick-cell.l4.r4').should('contain', 'Cypress-test-VN-append')
      cy.contains('.slick-cell.l2.r2', 'Cypress-test-VN-auto-backend-lr')
      .closest('.slick-row') 
      .find('.fa.fa-cog.icon-only.bigger-110.grid-action-dropdown')
      .click()
